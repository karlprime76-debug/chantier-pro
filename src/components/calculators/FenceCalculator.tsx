...
export function FenceCalculator() {
  ...
  return (
    <div className="grid gap-4">
      ...
      <Card>
        ...
        {output ? (
          <div className="grid gap-2 rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-white/70">
            <div>
              <span className="text-white/55">Nombre poteaux:</span> {output.postsCount}
            </div>
            <div>
              <span className="text-white/55">Surface mur:</span> {output.wallSurfaceM2} m²
            </div>
            <div>
              <span className="text-white/55">Agglos:</span> {output.blocksCount}
            </div>
            <div>
              <span className="text-white/55">Béton poteaux:</span> {output.postsConcreteVolumeM3} m³
            </div>
            <div>
              <span className="text-white/55">Béton longrine:</span> {output.footingConcreteVolumeM3} m³
            </div>
            <div>
              <span className="text-white/55">Béton total:</span> {output.totalConcreteVolumeM3} m³
            </div>
            <div>
      ...
      <div className="text-xs text-white/50">Modèle simplifié (MVP) : adapte sections, fondations et ferraillage selon sol et normes.</div>
    </div>
  );
}
