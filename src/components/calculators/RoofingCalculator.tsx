...
export function RoofingCalculator() {
  ...
  return (
    <div className="grid gap-4">
      ...
      <Card>
        ...
        {output ? (
          <div className="grid gap-2 rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-white/70">
            <div>
              <span className="text-white/55">Surface au sol (avec débords):</span> {output.planAreaM2} m²
            </div>
            <div>
              <span className="text-white/55">Surface toiture (avec pente + perte):</span> {output.roofAreaM2} m²
            </div>
            <div>
              <span className="text-white/55">Tôles/panneaux:</span> {output.panelsCount}
            </div>
            <div>
              <span className="text-white/55">Accessoires:</span> ~{Math.round(output.roofAreaM2 * output.accessoriesFactor)} unités (estimation)
            </div>
          </div>
        ) : (
      ...
      <div className="text-xs text-white/50">Les accessoires (vis, faîtières, rives) sont une estimation simple à ajuster selon le type de toiture.</div>
    </div>
  );
}
