...
import {
  computePlancherPoutrellesHourdis,
  PlancherPoutrellesHourdisInputSchema,
  type PlancherPoutrellesHourdisOutput,
} from "@/lib/calculators/plancherPoutrellesHourdis";
...
export function PlancherPoutrellesHourdisCalculator() {
  ...
  const [output, setOutput] = useState<PlancherPoutrellesHourdisOutput | null>(null);
  const [error, setError] = useState<string | null>(null);

  function handleCompute() {
    setError(null);

    const parsed = PlancherPoutrellesHourdisInputSchema.safeParse({
    ...
    try {
      setOutput(computePlancherPoutrellesHourdis(parsed.data));
    } catch {
      setOutput(null);
    ...
    <div className="grid gap-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Plancher poutrelles-hourdis (estimation).</CardDescription>
        </CardHeader>
      ...
      <Card>
        ...
        {output ? (
          <div className="grid gap-2 rounded-2xl border border-[var(--app-card-border)] bg-[color-mix(in_oklab,var(--app-card),transparent_10%)] p-4 text-sm text-[var(--app-text-muted)]">
            ...
            <div>
              <span className="text-[color-mix(in_oklab,var(--app-text),transparent_55%)]">Béton compression :</span> {output.compressionConcreteWithWasteM3} m³ (avec perte)
            </div>
          </div>
        ) : (
