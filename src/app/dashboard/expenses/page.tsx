...
export default async function ExpensesPage() {
  ...
  return (
    <div className="grid gap-6">
      <div>
        <h1 className="text-2xl font-extrabold tracking-tight text-white">Dépenses</h1>
        <p className="mt-1 text-sm text-white/60">Suis les achats, fournisseurs et validations.</p>
      </div>
          ...
          {expenses.length === 0 ? (
            ...
            expenses.map((e) => (
              <div key={e.id} className="rounded-xl border border-white/10 bg-black/20 p-4">
                ...
                <div className="mt-2 text-xs text-white/55">Statut: {e.status}</div>
              </div>
            ))
          )}
        </div>
